...
{
    kind_        = cfg_.kind;
    alphaAttack_ = computeAlpha(cfg_.attackSec, sampleRate);

    if (cfg_.kind == TimingKind::Fixed) {
        alphaRelease_ = computeAlpha(cfg_.releaseSec, sampleRate);
    } else {
        // AutoRelease: compute both parallel release branches.
        alphaFast_ = computeAlpha(cfg_.fastReleaseSec, sampleRate);
        alphaSlow_ = computeAlpha(cfg_.slowReleaseSec, sampleRate);
    }
}
...
void RectifierDetector::setConfig(RectifierDetectorConfig cfg) noexcept
{
    cfg_ = cfg;
}
...
float RectifierDetector::processSample(float sample) noexcept
{
    ...
    const float rect = std::abs(Analog::sampleToVolts(sample));
