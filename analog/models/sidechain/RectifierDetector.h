...
///
/// Time constants are specified explicitly (in seconds) rather than referencing
/// a plugin-specific preset table, keeping this class reusable across projects.
struct RectifierDetectorConfig {
    /// Fixed vs. programme-dependent (AutoRelease) behaviour.
    TimingKind kind           = TimingKind::Fixed;

    /// Attack time constant (s).  Controls how quickly the envelope rises.
    double attackSec          = 0.0002;

    /// Release time constant (s).  Used when kind = Fixed.
    double releaseSec         = 0.30;

    /// Fast-branch release time constant (s).  Used when kind = AutoRelease.
    double fastReleaseSec     = 0.050;

    /// Slow-branch release time constant (s).  Used when kind = AutoRelease.
    double slowReleaseSec     = 10.0;
};
...
///   3. Attack/release RC smoothing — behaviour depends on the configured kind:
///
///      Fixed:
...
///
/// The smoothing coefficients are recomputed from the config time constants and
...
public:
    ...
    /// A subsequent prepare() call is required to update the smoothing coefficients.
    void setConfig(RectifierDetectorConfig cfg) noexcept;

    /// Return the current configuration.
    [[nodiscard]] const RectifierDetectorConfig& config() const noexcept { return cfg_; }
...
private:
    ...
    double alphaRelease_ = 0.0;  ///< Smoothing coefficient for the release phase (Fixed only).
    ...
    /// Cached kind — set in prepare() so processSample() doesn't branch on cfg_.kind.
